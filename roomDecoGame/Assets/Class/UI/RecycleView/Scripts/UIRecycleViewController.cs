...
namespace UI
{
    ...
    public class UIRecycleViewController<T> : MonoBehaviour
    {
            ...
            if (cells.Count < 1)
            {
                ...
                for (int i = 0; i < tableData.Count; i++)
                {
                       ...
                       (cellBottom.y <= visibleRect.y && cellBottom.y >= visibleRect.y - visibleRect.height))
                    {
                        ...
                        cell.Top = cellTop;
                        Debug.Log("비지블 테스트 1: " + visibleRect.y + "    " + visibleRect.height);
                        break;
                    }
        ...
        protected virtual float GetCellHeightAtIndex(int index)
        {
            ...
            return 300; //cellBase.GetComponent<RectTransform>().sizeDelta.y;
        }
        ...
        private void SetFillVisibleRectWithCells()
        {
            ...
            {
                Debug.Log("비지블 테스트 2: " + visibleRect.y + "    " + visibleRect.height);
                Debug.Log("비지블 테스트 3: " + nextCellTop.y);
